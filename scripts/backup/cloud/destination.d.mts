...
export function makeTokenProvider(a: { provider: string; clientId: string; refreshToken: string; onRotate?: (rt: string) => void }): { getAccessToken: (force?: boolean) => Promise<string>; getRefreshToken: () => string }
export function fetchRetry(url: string, opts?: Record<string, unknown>, cfg?: { attempts?: number }): Promise<{ status: number; ok: boolean; headers: { get: (k: string) => string | null }; json: () => Promise<unknown>; body?: unknown }>
...
export function createDestination(a: { provider: string; clientId: string; refreshToken: string; folderId?: string; onRotate?: (rt: string) => void }): Destination
