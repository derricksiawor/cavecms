...
const BASE_SHAPES_FOR_BLOCK: Record<string, FieldShape[]> = {
  hero: [
    ...
    { kind: 'string', key: 'subtitle', label: 'Subtitle', maxLength: TEXT_MAX.short },
    { kind: 'media', key: 'image', label: 'Hero image' },
    { kind: 'cta', key: 'cta', label: 'Call to action' },
  ],
  services_intro: [
    { kind: 'string', key: 'title', label: 'Title', maxLength: TEXT_MAX.title },
    { kind: 'richtext', key: 'body_richtext', label: 'Intro', maxLength: TEXT_MAX.richtextShort },
  ],
  featured_projects: [
    { kind: 'string', key: 'title', label: 'Heading', maxLength: TEXT_MAX.title },
    {
      kind: 'select',
      key: 'layout',
      label: 'Layout',
  ...
  about_history: [
    { kind: 'string', key: 'title', label: 'Title', maxLength: TEXT_MAX.title },
    { kind: 'richtext', key: 'body_richtext', label: 'Body', maxLength: TEXT_MAX.richtextLong },
    { kind: 'media', key: 'image', label: 'Image' },
  ],
  cta: [
    { kind: 'string', key: 'title', label: 'Title', maxLength: TEXT_MAX.title },
    { kind: 'string', key: 'body', label: 'Body', maxLength: TEXT_MAX.body, multiline: true },
  ...
  quote: [
    ...
    { kind: 'string', key: 'attribution', label: 'Attribution', maxLength: TEXT_MAX.caption },
    { kind: 'string', key: 'attribution_title', label: 'Attribution title', maxLength: TEXT_MAX.caption },
  ],
  button: [
    { kind: 'string', key: 'text', label: 'Button text', maxLength: TEXT_MAX.ctaText, placeholder: 'e.g. Schedule a tour' },
    { kind: 'string', key: 'href', label: 'Link target (required)', maxLength: TEXT_MAX.url, placeholder: '/contact  or  https://…', help: 'Allowed schemes: http, https, mailto, tel, /relative paths.' },
    { kind: 'boolean', key: 'openInNew', label: 'Open in a new tab', help: 'Recommended for external links.' },
    {
      ...
      label: 'Variant',
      options: [
        { value: 'primary', label: 'Primary (copper)' },
  ...
  divider: [
    {
      ...
      help: 'Double needs a 4px thickness to render visibly.',
    },
    {
      kind: 'select',
      key: 'width',
    ...
    },
    {
      ...
      key: 'alignment',
      label: 'Alignment',
      help: 'No effect when width is "Full" — the rule already spans the container.',
      options: [
        { value: 'left', label: 'Left' },
        { value: 'center', label: 'Center' },
        { value: 'right', label: 'Right' },
  ...
  spacer: [
    {
      ...
      options: [
        ...
        { value: 'sm', label: 'S (16px)' },
        { value: 'md', label: 'M (32px)' },
        { value: 'lg', label: 'L (64px)' },
  ...
  icon_box: [
    {
      ...
      key: 'icon',
      label: 'Icon',
      help: 'Search the full Lucide library — picked icons render with the box accent.',
    },
    { kind: 'string', key: 'headline', label: 'Headline', maxLength: TEXT_MAX.title },
    { kind: 'string', key: 'body', label: 'Body', maxLength: TEXT_MAX.body, multiline: true },
    { kind: 'link', key: 'link', label: 'Link (optional — whole box is clickable)' },
    {
      kind: 'select',
      key: 'alignment',
      label: 'Alignment',
    ...
    },
    {
      ...
      label: 'Accent',
      options: [
        { value: 'copper-filled', label: 'Copper filled' },
        { value: 'copper-outline', label: 'Copper outline' },
        { value: 'cream-tint', label: 'Cream tint' },
      ],
    },
  ],
  accordion: [
    {
      ...
      maxItems: 20,
      itemTitle: (item, i) => {
        const title = typeof item.title === 'string' && item.title.trim() !== '' ? item.title : `Item ${i + 1}`
        return title.length > 32 ? `${title.slice(0, 32)}…` : title
      },
      itemFields: [
        { kind: 'string', key: 'title', label: 'Title', maxLength: TEXT_MAX.caption, placeholder: 'Question or section title' },
        { kind: 'richtext', key: 'body_richtext', label: 'Body', maxLength: TEXT_MAX.richtextShort, placeholder: 'Answer or section body' },
      ],
    },
    { kind: 'number', key: 'default_open_index', label: 'Default open index', min: 0, help: 'Leave blank for none open. Pre-expanding one item improves SEO crawl visibility for that item.' },
    { kind: 'boolean', key: 'allow_multiple', label: 'Allow multiple items open at once', help: 'When off, opening one item closes any other open item (Chrome 120+ / Safari 17+ / Firefox 121+).' },
  ],
  icon_list: [
    {
      kind: 'object_array',
      ...
      maxItems: 24,
      itemTitle: (item, i) => {
        const label = typeof item.label === 'string' && item.label.trim() !== '' ? item.label : `Item ${i + 1}`
    ...
    },
    {
      ...
      label: 'Style',
      options: [
        { value: 'copper-inline', label: 'Copper inline' },
        { value: 'copper-circle', label: 'Copper circle' },
      ],
    },
  ],
  tabs: [
    {
      ...
      maxItems: 12,
      itemTitle: (item, i) => {
        const label = typeof item.label === 'string' && item.label.trim() !== '' ? item.label : `Tab ${i + 1}`
        return label.length > 32 ? `${label.slice(0, 32)}…` : label
      },
      itemFields: [
        { kind: 'string', key: 'label', label: 'Tab label', maxLength: TEXT_MAX.caption, placeholder: 'Short tab label' },
        { kind: 'richtext', key: 'body_richtext', label: 'Tab body', maxLength: TEXT_MAX.richtextShort, placeholder: 'Tab content' },
      ],
    },
    { kind: 'number', key: 'default_tab_index', label: 'Default active tab', min: 0, help: 'Defaults to the first tab. Clamps to a valid index if you remove tabs.' },
  ],
  heading: [
    { kind: 'string', key: 'text', label: 'Text', maxLength: TEXT_MAX.title, placeholder: 'Type a heading…' },
    {
      kind: 'select',
      key: 'level',
      label: 'Level',
      ...
      help: 'Use H1 only when this is the page’s primary heading.',
    },
    {
      kind: 'select',
      key: 'alignment',
  ...
  alert: [
    {
      ...
      help: 'Drives the icon; the surface stays the same near-black card with copper accent.',
    },
    { kind: 'string', key: 'title', label: 'Title', maxLength: TEXT_MAX.caption, placeholder: 'A short, action-oriented title' },
    { kind: 'richtext', key: 'body_richtext', label: 'Body', maxLength: TEXT_MAX.richtextShort, placeholder: 'Optional supporting copy' },
    {
      kind: 'boolean',
      key: 'dismissible',
      label: 'Dismissible',
      help: 'Show a close button. Visitors who dismiss won’t see it again until you edit the alert content.',
    },
  ],
  social_icons: [
    // object_array (not social_link_array) because the SocialIcons
    // schema's `platform` enum is the EXACT 7-value list matching
    // our shipped brand-mark SVGs (lib/blocks/SocialIcons + public/
    // icons/social/). The existing social_link_array's SocialLinkRow
    // emits free-string platform names ("Instagram", "X (Twitter)",
    // "Pinterest" / "Threads") that don't round-trip through this
    // schema's lowercase + curated enum. Using object_array with a
    // strict-enum select keeps the form-emitted shape aligned with
    // the Zod schema.
    {
      kind: 'object_array',
      key: 'items',
      ...
      maxItems: 7,
      itemTitle: (item, i) => {
        const p = typeof item.platform === 'string' ? item.platform : ''
        if (!p) return `Profile ${i + 1}`
      ...
      itemFields: [
        ...
        },
        {
          ...
          key: 'new_window',
          label: 'Open in a new tab',
          help: 'Default: yes. Set off to navigate in the current tab (rare; usually only for internal cross-tab share links).',
        },
      ],
    },
    {
      ...
      options: [
        ...
        { value: 'naked', label: 'Naked (no frame)' },
      ],
    },
    {
      kind: 'select',
  ...
  star_rating: [
    {
      ...
      max: 5,
      step: 0.5,
      help: 'Use half-star increments. The visual rounds to the nearest 0.5.',
    },
    { kind: 'string', key: 'label', label: 'Label', maxLength: TEXT_MAX.caption, placeholder: 'e.g. Client rating' },
    {
      kind: 'number',
      key: 'review_count',
      label: 'Review count',
      min: 0,
      step: 1,
      help: 'Shown as "(412 reviews)" next to the score.',
    },
    {
      kind: 'select',
      key: 'alignment',
  ...
  stats_row: [
    {
      ...
      maxItems: 6,
      itemTitle: (item, i) => {
        const label = typeof item.label === 'string' && item.label.trim() !== '' ? item.label : `Counter ${i + 1}`
        return label.length > 32 ? `${label.slice(0, 32)}…` : label
      },
      itemFields: [
        { kind: 'number', key: 'value', label: 'Target value', step: 1 },
        { kind: 'string', key: 'prefix', label: 'Prefix', maxLength: 8, placeholder: 'e.g. $' },
        { kind: 'string', key: 'suffix', label: 'Suffix', maxLength: 8, placeholder: 'e.g. M+ or %' },
        { kind: 'string', key: 'label', label: 'Label', maxLength: TEXT_MAX.caption, placeholder: 'e.g. Residences sold' },
        { kind: 'string', key: 'helper_text', label: 'Helper text', maxLength: TEXT_MAX.caption, placeholder: 'Optional sub-label' },
        {
          kind: 'number',
          key: 'duration_ms',
          label: 'Animation (ms)',
          ...
          max: 10000,
          step: 100,
          help: 'Time to climb from 0 to the target. 1800ms reads as premium; longer feels broken.',
        },
      ],
    },
    {
      ...
      label: 'Layout',
      options: [
        { value: 'solo', label: 'Solo (single tile)' },
        { value: '2up', label: '2-up grid' },
        { value: '3up', label: '3-up grid' },
        { value: '4up', label: '4-up grid' },
      ],
      help: 'Solo collapses to one centred tile regardless of how many counters you’ve added.',
    },
  ],
  testimonial: [
    {
      ...
      multiline: true,
      placeholder: 'A short, glowing testimonial from a recent client.',
    },
    { kind: 'string', key: 'attribution', label: 'Attribution (name)', maxLength: TEXT_MAX.caption },
    { kind: 'string', key: 'role', label: 'Role / company', maxLength: TEXT_MAX.caption },
    { kind: 'media', key: 'image', label: 'Photo (optional)' },
    {
      kind: 'number',
      key: 'project_id',
      ...
      min: 1,
      step: 1,
      help: 'Link to a project page. Find the ID in the projects admin list.',
    },
    {
      kind: 'select',
      key: 'alignment',
  ...
  video_embed: [
    {
      ...
      help: 'YouTube embed URL or Vimeo player URL only. Other hosts will be rejected.',
    },
    {
      kind: 'select',
      key: 'aspect_ratio',
