  ...
  return {
    ...
    sectionAndColumnMeta: {
      ...
      gradientsAndExactType: {
        ...
        buttonControls: "lx_action: variant + size + alignment, fillColor (token or #hex solid, e.g. white button — hover overrides still work alongside it), radius (px) OR per-corner radiusTopLeft/TopRight/BottomRight/BottomLeft, backgroundGradient (fill) / textGradient (label), hover (hoverFillColor/hoverTextColor/hoverScale/transitionMs), elevation ['none'|'shadow'|'pulse'] — 'none' = a FLAT brand button (primary-gold defaults to 'pulse': shadow + champagne glow).",
        headingHighlight: "lx_heading: highlightText (a substring of `text`) + highlightColor (token or #hex, default champagne) renders a two-tone heading — the first occurrence of highlightText is wrapped in an accent span (e.g. text:'Choose Tomorrow, Today', highlightText:'Today').",
        marqueeControls: "lx_marquee: speed ['slow'|'medium'|'fast'|'static'] — 'static' renders the logos/text as a centered, NON-animated row (a still 'trusted by' strip).",
        formSubmitControls: "lx_form submit button: submitFillColor + submitTextColor (token or #hex), submitFullWidth (bool — stretch across the form), submitRadius (px, unset = full pill). Decoupled from the field-text colour — no customCss token hacks needed.",
        afterSubmitDelivery: "lx_form `actions[]` (max 8) deliver a gated PDF after a successful submit. Each action: { kind:'deliver_file', file:{media_id,alt}, mode, emailSubject?, emailBody? }. mode = 'email' (default — emails a signed link that expires in 7 days), 'attach' (attaches the PDF itself to the confirmation email; PDF only, ≤10MB — a missing/oversized/non-PDF file gracefully degrades to an emailed link, and absent a valid email/SMTP to an on-screen download button), 'instant' (shows a Download button on the success screen), or 'manual' (no auto-delivery; the lead is saved + the team notified for them to send it). `file` must point at an uploaded PDF media row. Default stays 'email'; attach is the lead-magnet 'send the brochure as an attachment, not a link' flow.",
    ...
    branding: {
      ...
      site_header: {
        purpose: 'Logo, brand text, top nav, primary CTA — plus optional brand-exact CTA + nav colour overrides, and the overlay (transparent-over-hero) header mode with its own logo.',
        shape: { brandText: 'string', logo: '{ media_id, alt } | null', logoMaxHeight: '24–96', theme: '"cream"|"obsidian"|"ivory"|"champagne"|"bone"', headerMode: '"solid" (default) | "overlay" — overlay floats the bar transparent over the hero and turns it into the solid themed bar on scroll', overlayLogo: '{ media_id, alt } | null (optional — usually a WHITE version of the logo, shown ONLY while the bar is transparent; the main `logo` takes over once solid. Unset = main logo in both states)', overlayTone: '"light" (default — white text over a dark hero) | "dark" (near-black over a light hero); only affects the transparent state', navItems: '[{ label, href, children?:[{label,href}] }] (max 6)', primaryCta: '{ text, href, openInNew? } | null  (NOTE: field is `text`, not `label`)', ctaFillColor: '#hex (optional — CTA pill background; unset = theme default)', ctaTextColor: '#hex (optional)', ctaHoverFillColor: '#hex (optional — hover works alongside the fill)', ctaHoverTextColor: '#hex (optional)', ctaRadius: '0–64 px (optional; unset = full pill)', navColor: '#hex (optional — nav link colour)', navActiveColor: '#hex (optional — active link colour; also the rest links’ hover colour)' },
      },
      footer: {
        purpose: 'Footer logo, link columns, newsletter, copyright, legal links — plus optional accent + Subscribe-button colour overrides.',
        shape: { tagline: 'string', theme: 'header-theme token', logo: '{ media_id, alt } | null', columns: '[{ label, links:[{ text, href }] }]', newsletterEnabled: 'bool (default true — false removes the whole newsletter column and the grid reflows)', newsletterHeading: 'string', newsletterBody: 'string', newsletterCtaLabel: 'string', copyright: 'string', legalLinks: '[{ text, href }]', accentColor: '#hex (optional — column + newsletter heading accent)', ctaFillColor: '#hex (optional — Subscribe button background)', ctaTextColor: '#hex (optional)', ctaHoverFillColor: '#hex (optional)', ctaHoverTextColor: '#hex (optional)' },
      },
      typography: {
