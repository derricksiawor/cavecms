...
export function getCapabilities() {
  ...
  return {
    ...
    branding: {
      ...
      site_header: {
        purpose: 'Logo, brand text, top nav, primary CTA — plus optional brand-exact CTA + nav colour overrides.',
        shape: { brandText: 'string', logo: '{ media_id, alt } | null', logoMaxHeight: '24–96', theme: '"cream"|"obsidian"|"ivory"|"champagne"|"bone"', navItems: '[{ label, href, children?:[{label,href}] }] (max 6)', primaryCta: '{ text, href, openInNew? } | null  (NOTE: field is `text`, not `label`)', ctaFillColor: '#hex (optional — CTA pill background; unset = theme default)', ctaTextColor: '#hex (optional)', ctaHoverFillColor: '#hex (optional — hover works alongside the fill)', ctaHoverTextColor: '#hex (optional)', ctaRadius: '0–64 px (optional; unset = full pill)', navColor: '#hex (optional — nav link colour)', navActiveColor: '#hex (optional — active link colour; also the rest links’ hover colour)' },
      },
      footer: {
