...
| Media | Local `/opt/bwc/uploads` (chmod 750, owned by app user), `public/uploads` is a symlink, `sharp` generates thumb/md/lg/og variants on upload, atomic temp+rename pipeline |
...
Shared droplet — BWC piggybacks on existing nginx and native MariaDB. Email delivery is handled by the user manually outside this project; the app only reads SMTP env vars and sends with nodemailer.
...
[DigitalOcean droplet — shared with other apps, Ubuntu, UFW firewalled]
   ...
   │     Process user: `bwc` (system account, no shell)
   ...
   │                            env_file: /etc/bwc/env.<env>  (symlinked into release)
   │     UV_THREADPOOL_SIZE=8 (sharp), NODE_OPTIONS='--max-old-space-size=1024'
   │
   ├─ MariaDB (existing, native systemd service)
   │     DB `bwc`, user `bwc_user` GRANTED: SELECT/INSERT/UPDATE/DELETE/INDEX/CREATE/ALTER/REFERENCES ON bwc.*
   │     Migrations user `bwc_migrator` separate (above + DROP) — used only by deploy
   │     time_zone = '+00:00' (UTC) on the connection
   │
   ├─ /opt/bwc/                              (chown bwc:bwc, chmod 750)
   ...
   │
   ├─ /backup/bwc/  (chown bwc:backup, chmod 750, encrypted at rest via age)
...
All shell scripts begin with `set -euo pipefail; IFS=$'\n\t'` and a `trap 'echo "FAIL at line $LINENO"' ERR`. No `sed`. No `eval`. All variable expansions double-quoted. mysqldump credentials read from `/root/.bwc-backup.cnf` (chmod 600) `[client]` block — never on the command line.
...
3. **Assert same-filesystem invariant** before any write: `fs.statSync('/opt/bwc/uploads/.tmp').dev === fs.statSync('/opt/bwc/uploads/originals').dev`. setup.sh enforces all four upload dirs live on one filesystem.
...
5. Write all variant bytes to `/opt/bwc/uploads/.tmp/<filename_uuid>/<variant>.<ext>` for images, or `/opt/bwc/uploads/.tmp/<filename_uuid>.pdf` for PDFs.
...
8. Atomic `rename(2)` each from `.tmp/` → final paths (`/opt/bwc/uploads/originals/...`, `/opt/bwc/uploads/variants/...` or `/opt/bwc/uploads/brochures-private/...`). All on the same filesystem per step 3.
...
**File mode**: PM2 starts with `umask 027` so newly written files are `640` (owner-rw, group-r). Directories `750`. Group `bwc` includes `www-data` so nginx can read.
...
**Cleanup**: a boot sweeper removes ALL `/opt/bwc/uploads/.tmp/*` directories unconditionally (any leftover is by definition orphaned from a prior process). Weekly `scripts/verify-media-refs.ts` cross-checks `media` rows against on-disk files.
...
Token URL is sent in the brochure auto-reply email; the PDF lives only in `/opt/bwc/uploads/brochures-private/` (never served by nginx). `BROCHURE_SECRET` rotation invalidates all outstanding tokens immediately.
...
SignJWT({ resource_type: 'project', resource_id, preview_epoch })
  ...
  .setIssuer('bwc.preview').setAudience('bwc.preview')
...
new SignJWT({ tv: user.tokens_valid_after_ms })   // role NOT in claim
  .setProtectedHeader({ alg: 'HS256' })
  .setIssuer('bwc.cms').setAudience('bwc.web')
...
const { payload, protectedHeader } = await jwtVerify(token, secret, {
  algorithms: ['HS256'],
  issuer: 'bwc.cms', audience: 'bwc.web',
  clockTolerance: '5s',
})
...
Cookie: `__Host-bwc_session`, `httpOnly: true, secure: true (prod), sameSite: 'strict', path: '/', maxAge: JWT_TTL_SECONDS`. No `Domain`. Sliding renewal as in §6.
...
- Token returned in JSON AND set as `__Host-bwc_csrf` cookie (`Secure, SameSite=Strict, Path=/`, **not** httpOnly).
...
- The page server-renders the form with a hidden `<input name="csrf">` populated from a freshly-generated value `preNonce`, AND sets a parallel `__Host-bwc_pre_csrf=preNonce` cookie (httpOnly, Secure, SameSite=Strict, 15 min TTL).
...
**Login response cookie rotation**: `POST /api/auth/login` 200 response sets both the new `__Host-bwc_session` AND a freshly minted `__Host-bwc_csrf` cookie (bound to the new jti) AND returns the new CSRF token in the response body so the client can prime its in-memory copy without an immediate `/api/csrf` round-trip. The pre-auth cookie is cleared.
...
Brochure PDFs live in `/opt/bwc/uploads/brochures-private/<uuid>.pdf` — **not** symlinked into `public/` and **not** served by nginx. Access only via `/api/brochure/[token]`:
...
|---|---|:-:|:-:|---|---|---|
| `__Host-bwc_session` | JWT session | ✓ | ✓ | Strict | `/` | JWT_TTL |
| `__Host-bwc_csrf` | CSRF echo | ✗ | ✓ | Strict | `/` | CSRF_TTL |
| `__Host-bwc_pre_csrf` | Pre-auth CSRF | ✓ | ✓ | Strict | `/` | 15 min |
| `__Host-bwc_edit_mode` | Edit overlay toggle | ✓ | ✓ | Strict | `/` | JWT_TTL |

Subdomain isolation: staging and apex have distinct `__Host-` cookies (no `Domain` allowed → automatic isolation).

### Multi-tab session

`BroadcastChannel('bwc-auth')`: on login/logout/role-change events the active tab posts `{ type: 'invalidate' }`. All open tabs call `router.refresh()` and re-render auth-gated UI. Server components reading the session cookie use `cookies()` (uncached per Next.js docs).
...
- PDFs: no transformation; placed in `/opt/bwc/uploads/brochures-private/` (private; served only via `/api/brochure/[token]`).
...
- Backup: `/opt/bwc/uploads/{originals,variants}` rsynced daily to `/backup/bwc/uploads/`. Brochures-private rsynced separately to a key-restricted backup path. Both encrypted via `age`.
...
2. **Push to `main`:** above + production build:
   ...
   - rsync to droplet's `/opt/bwc/releases/<sha>/` via deploy-only SSH key.
...
scripts/
  ...
  nginx/
    bwc.conf.template                 # processed by setup.sh
public/
  brand/
    logo.svg
    favicon.ico
  fonts/
    *.woff2
  uploads -> /opt/bwc/uploads         # symlink (gitignored)
tests/
  unit/
  integration/
  e2e/
...
docs/
  specs/2026-05-10-bwc-rebuild-design.md
...
- **Migration runner uses the `bwc_migrator` DB user from `migrator-bundle/`**, NOT the app's pool.
...
- Failed migration recovery: `scripts/migrate-recover.sh` prompts before marking a partially-applied migration as applied (after manual SQL fix). Pre-deploy DB snapshot in `/backup/bwc/pre-deploy/<sha>.sql.age` provides rollback; the rollback script auto-restores when the rolled-back release's `schema-fingerprint.txt` does not match the live DB fingerprint.

### Scripts (all idempotent, `set -euo pipefail; IFS=$'\n\t'`, `trap 'echo "FAIL at line $LINENO"' ERR`)

- `scripts/setup.sh` — one-time per box. Creates the `bwc` system user (no shell), adds `www-data` to the `bwc` group, creates the `bwc` DB + `bwc_user` + `bwc_migrator` users with scoped grants, mkdirs `/opt/bwc/{releases,uploads/{originals,variants,brochures-private,.tmp},static-pool}` and `/backup/bwc/{db,uploads,pre-deploy}` with correct ownership (`bwc:bwc 750`) and asserts they all live on the same filesystem (`stat -c %d` equality), drops nginx server blocks (validates with `nginx -t` BEFORE reload), installs the certbot deploy hook (which itself runs `nginx -t && systemctl reload nginx`), prompts for and validates `LOGIN_PATH`, installs `pm2-logrotate`, configures `pm2 startup systemd`, installs systemd timers (db backup at 03:00 + uploads backup at 03:30 + cron-purge at 04:00 with `After=` ordering between them + disk-check hourly + media-refs verify weekly, all with `RandomizedDelaySec=15min`, `Persistent=true`, `IOSchedulingClass=idle` on backups, and `OnFailure=bwc-alert@%n.service`).
- `scripts/seed-admin.sh` — one-time per box, run by operator after `setup.sh`. Prompts interactively for `INITIAL_ADMIN_EMAIL`, `INITIAL_ADMIN_NAME`, `INITIAL_ADMIN_PASSWORD` (never persisted to `/etc/bwc/env.*`). Invokes the migrator bundle to insert the seed row with `must_rotate_password = TRUE`.
- `scripts/deploy.sh` — every deploy. Holds `flock -n /var/lock/bwc-deploy.lock`. Sequence:
  1. Preflight: compute `required_free = size(latest_artifact) × 2 + size(latest_db_dump) + 500MB`; reject if `df --output=avail /opt /backup` < required. MariaDB reachable check. Current `/healthz` returns 200 (skipped on first deploy). Reject if `/var/lib/bwc/deploy.blocked` exists.
  ...
  4. rsync prebuilt CI artifact into `/opt/bwc/releases/<sha>/`.
  5. Symlink `/etc/bwc/env.<env> → /opt/bwc/releases/<sha>/.env.local` (chmod 600 preserved).
  6. Pre-deploy DB snapshot: `mysqldump --defaults-extra-file=… --single-transaction --quick --skip-lock-tables --routines --triggers --events --set-gtid-purged=OFF bwc | gzip | age -r $(cat /etc/bwc/backup.pub) > /backup/bwc/pre-deploy/<sha>.sql.age.partial`; verify all `${PIPESTATUS[@]}` are `0`; verify size ≥ 50% of prior snapshot (anomaly guard); rename `.partial` → final.
  7. Run forward migrations via `migrator-bundle/`: `cd /opt/bwc/releases/<sha> && node migrator-bundle/run.js`. Migrator updates `schema_fingerprint`.
  8. **Merge new static assets into static pool**: `rsync -a /opt/bwc/releases/<sha>/.next/static/ /opt/bwc/static-pool/.next/static/` (additive; old chunks remain for at least 7 days). `nginx` aliases `/_next/static` from `/opt/bwc/static-pool/.next/static/` so clients with stale HTML still resolve their chunks.
  9. Atomic `ln -sfn /opt/bwc/releases/<sha> /opt/bwc/current`.
  10. `pm2 startOrReload /opt/bwc/current/ecosystem.config.cjs --update-env`.
...
- `scripts/db-backup.sh` — `mysqldump --defaults-extra-file=/root/.bwc-backup.cnf --single-transaction --quick --skip-lock-tables --routines --triggers --events --set-gtid-purged=OFF bwc | gzip | age -r $(cat /etc/bwc/backup.pub) > /backup/bwc/db/<date>.sql.age.partial`; check `${PIPESTATUS[@]}`; size-anomaly guard (≥ 50% of yesterday); rename `.partial` → final; on failure remove `.partial`. `flock`-serialized against deploy lock to avoid races. `ionice` class idle. Weekly: `age --decrypt --identity=<test-identity-from-stdin>` round-trip test on the latest dump (drill).
- `scripts/uploads-backup.sh` — `rsync -a --delete-after /opt/bwc/uploads/{originals,variants,brochures-private}/ /backup/bwc/uploads/`. Encrypted via age-on-rsync-target only if `BACKUP_RSYNC_REMOTE` set to an `rclone` destination.
...
- `scripts/disk-check.sh` — hourly. When `df --output=pcent /` ≥ 80 → warn to `journald`. ≥ 90 → page (email + create `/var/lib/bwc/deploy.blocked`). Cleared manually after disk pressure resolved.
...
### nginx server block (template under `scripts/nginx/bwc.conf.template`)

Snippets included by each 443 block: `bwc-proxy.conf` (proxy headers), `bwc-tls.conf` (TLS hardening), `bwc-compression.conf` (gzip + brotli_static).
...
server {
  ...
  log_format bwc_json escape=json '{"ts":"$time_iso8601","ip":"$remote_addr","method":"$request_method","path":"$request_uri","status":$status,"bytes":$body_bytes_sent,"rt":$request_time,"ua":"$http_user_agent","ref":"$http_referer"}';
  ...
  location = /__LOGIN_PATH__   { access_log off; proxy_pass http://127.0.0.1:3040; include /etc/nginx/snippets/bwc-proxy.conf; }
  location = /api/auth/login   { access_log off; proxy_pass http://127.0.0.1:3040; include /etc/nginx/snippets/bwc-proxy.conf; }
...
`/etc/nginx/snippets/bwc-proxy.conf`:
```
...
`/etc/nginx/snippets/bwc-tls.conf`:
```
...
`/etc/nginx/snippets/bwc-compression.conf`:
```
gzip on;
gzip_vary on;
gzip_comp_level 6;
...
```
[Unit] Description=BWC nightly DB backup
...
- nginx logs `/var/log/nginx/bwc-*.log` rotated by logrotate (7d retention, compressed).
...
- DB: `mysqldump --single-transaction --quick --skip-lock-tables --routines --triggers --events --set-gtid-purged=OFF bwc | gzip | age -r $(cat /etc/bwc/backup.pub)` → `/backup/bwc/db/<date>.sql.age` (`.partial` rename pattern). `--single-transaction` avoids holding a global lock that would block writes for the OTHER apps sharing this MariaDB. Retention `DB_BACKUP_RETENTION_DAYS` (default 30).
...
Env files on the droplet live in `/etc/bwc/env.production` and `/etc/bwc/env.staging` (`chmod 600 root:bwc`). `deploy.sh --env <name>` symlinks the right file into the release dir as `.env.local` — this is the SINGLE env mechanism (no PM2 `env_file`; PM2 inherits the process env and Next.js loads `.env.local` natively). To rotate a secret: edit the central file, `pm2 reload bwc`.

**`INITIAL_ADMIN_*` are NEVER persisted to `/etc/bwc/env.production`.** They are prompted interactively by `scripts/seed-admin.sh` on first deploy only, used to insert the seed row, and discarded from the shell history (`set +o history` around the prompt). Subsequent restarts do not require these vars.
...
- `/opt/bwc/uploads` owned `bwc:bwc` chmod 750; brochures-private not served by nginx.
...
- Backup destination path on the droplet (`/backup/bwc/...` assumed).
...
All defaults sensible; override in `/etc/bwc/env.<env>` per environment. Boot-time validation throws on missing required vars. `NEXT_PUBLIC_*` vars are inlined at CI build time, not at runtime.
...
DATABASE_URL=mysql://bwc_user:CHANGEME@127.0.0.1:3306/bwc
DATABASE_MIGRATOR_URL=mysql://bwc_migrator:CHANGEME@127.0.0.1:3306/bwc
...
# First-deploy only — NEVER persisted to /etc/bwc/env.production. Prompted by scripts/seed-admin.sh and consumed once.
INITIAL_ADMIN_EMAIL=
INITIAL_ADMIN_NAME=
...
|---|---|---|---|
| `pm2 reload` on a single fork instance is effectively a brief restart, dropping in-flight requests | LOW | `kill_timeout: 8000` + nginx maintenance fallback (`error_page 502/503/504 → bwc-maint.html`) + `proxy_next_upstream`; deploys at low-traffic hours | Yes — true blue/green via two PM2 instances + DB-backed rate limits is v2 |
...
| `bwc_user` DB user has CREATE/ALTER on `bwc.*` (needed for some legacy migrations) | LOW | `bwc_migrator` is the only user with DROP; app-runtime queries via `bwc_user` only | Yes |
