# BWC Plan 02 — CMS Core (Blocks + Media)
...
export const POST = withError(async (req) => {
  ...
  const csrfHeader = req.headers.get('x-csrf-token') ?? ''
  const c = await cookies()
  if (csrfHeader !== c.get('__Host-bwc_csrf')?.value || !await verifyCsrf(csrfHeader, { jti: ctx.jti, sub: String(ctx.userId) })) {
    throw new HttpError(403, 'csrf_invalid')
  }
...
export const DELETE = withError(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const csrfHeader = req.headers.get('x-csrf-token') ?? ''
  const c = await cookies()
  if (csrfHeader !== c.get('__Host-bwc_csrf')?.value || !await verifyCsrf(csrfHeader, { jti: ctx.jti, sub: String(ctx.userId) })) throw new HttpError(403, 'csrf_invalid')
...
const UPLOADS_ROOT = process.env.UPLOADS_ROOT ?? '/opt/bwc/uploads'
...
- [ ] Manual: upload a JPG → 4 variants appear under `/opt/bwc/uploads/variants/`; `media.variants` JSON populated.
...
Plan saved to `docs/superpowers/plans/2026-05-10-bwc-02-cms-core.md`. Pick subagent-driven or inline execution. After Plan 02 green, proceed to Plan 03 (Inline Edit UX).
