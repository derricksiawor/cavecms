...
**Spec reference:** `docs/specs/2026-05-10-bwc-rebuild-design.md` (§1–§7, §15).
...
```bash
cd /Users/Derrick/portfolio/bwc
...
const ISS_SESSION = 'bwc.cms', AUD_SESSION = 'bwc.web'
...
export async function verifyPreviewJwt(
  ...
  const { payload, protectedHeader } = await jwtVerify(token, PREVIEW_KEY, {
    algorithms: ['HS256'], issuer: 'bwc.preview', audience: 'bwc.preview',
...
- [ ] **Step 4: run integration test (needs MariaDB up and `bwc_test` DB migrated). Commit:**
...
export async function requireAuth(): Promise<AuthContext> {
  ...
  const token = c.get('__Host-bwc_session')?.value
...
export async function middleware(req: NextRequest): Promise<NextResponse> {
  const token = req.cookies.get('__Host-bwc_session')?.value
...
export default async function LoginPage({ params }: { params: Promise<{ loginPath: string }> }) {
  ...
  const c = await cookies()
  c.set('__Host-bwc_pre_csrf', preNonce, {
...
export const POST = withError(async (req: Request) => {
  ...
  const csrfCookie = c.get('__Host-bwc_pre_csrf')?.value ?? ''

  c.delete('__Host-bwc_pre_csrf')                                      // always clear
  ...
  c.set('__Host-bwc_session', token, {
    httpOnly: true, secure: env.NODE_ENV === 'production',
    sameSite: 'strict', path: '/', maxAge: env.JWT_TTL_SECONDS,
  })
  c.set('__Host-bwc_csrf', csrf, {
...
export const POST = withError(async (req) => {
  ...
  c.delete('__Host-bwc_session'); c.delete('__Host-bwc_csrf')
...
const COMMIT = process.env.BWC_COMMIT ?? 'unknown'
...
- Successful login issues `__Host-bwc_session` + `__Host-bwc_csrf` cookies; `/admin` accessible.
...
Plan complete and saved to `docs/superpowers/plans/2026-05-10-bwc-01-foundation-auth.md`. Two execution options:
