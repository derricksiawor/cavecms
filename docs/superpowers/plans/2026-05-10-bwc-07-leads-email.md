...
**Architecture:** Each public form page server-renders a fresh `preNonce` into a hidden field + sets `__Host-bwc_pre_csrf` (httpOnly). POST validates body vs cookie + consumes the single-use nonce. After Zod + spam checks, INSERTs the lead + emails in one TX; queue runner sweeps and sends.
...
export async function ensurePublicPreCsrf(): Promise<string> {
  ...
  const existing = c.get('__Host-bwc_pre_csrf')?.value
  if (existing) return existing
  const v = issuePreCsrf()
  c.set('__Host-bwc_pre_csrf', v, {
...
export async function consumePublicPreCsrf(formValue: string): Promise<boolean> {
  const c = await cookies()
  const cookie = c.get('__Host-bwc_pre_csrf')?.value ?? ''
  c.delete('__Host-bwc_pre_csrf')
...
test('honeypot tripped → neutral response, no row', async ({ page }) => {
  ...
  const cookies = (await page.context().cookies()).find((c) => c.name === '__Host-bwc_pre_csrf')
