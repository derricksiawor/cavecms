...
**Architecture:** Releases land in `/opt/bwc/releases/<sha>/`; `current` symlink points to active. PM2 single fork + nginx maintenance fallback covers the brief reload window. Backups encrypted by age; restore drill runs monthly OFF the droplet. Logs JSON-escaped in nginx; PM2 logs rotated by pm2-logrotate.
...
read -r -s -p "New bwc_user password: " BWC_PW; echo
read -r -s -p "New bwc_migrator password: " MIG_PW; echo
...
if ! id bwc >/dev/null 2>&1; then
  useradd -r -s /usr/sbin/nologin -m -d /home/bwc bwc
fi
...
# 2. Directories
install -d -o bwc -g bwc -m 750 /opt/bwc /opt/bwc/releases /opt/bwc/uploads /opt/bwc/uploads/originals /opt/bwc/uploads/variants /opt/bwc/uploads/brochures-private /opt/bwc/uploads/.tmp /opt/bwc/static-pool/.next/static
install -d -o bwc -g backup -m 750 /backup /backup/bwc /backup/bwc/db /backup/bwc/uploads /backup/bwc/pre-deploy
install -d -o bwc -g bwc -m 750 /var/log/bwc /var/www
install -d -o root -g root -m 750 /etc/bwc /var/lib/bwc
...
CREATE USER IF NOT EXISTS 'bwc_user'@'%' IDENTIFIED BY '${BWC_PW}';
ALTER USER 'bwc_user'@'%' IDENTIFIED BY '${BWC_PW}';
GRANT SELECT, INSERT, UPDATE, DELETE ON bwc.* TO 'bwc_user'@'%';
CREATE USER IF NOT EXISTS 'bwc_migrator'@'%' IDENTIFIED BY '${MIG_PW}';
ALTER USER 'bwc_migrator'@'%' IDENTIFIED BY '${MIG_PW}';
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, INDEX, DROP, REFERENCES ON bwc.* TO 'bwc_migrator'@'%';
FLUSH PRIVILEGES;
SQL

# 4. Backup pubkey
echo "$AGE_PUB" > /etc/bwc/backup.pub
chmod 644 /etc/bwc/backup.pub

# 5. mysqldump credentials file (used by backup script)
cat > /root/.bwc-backup.cnf <<EOF
[client]
host=${DB_HOST}
user=bwc_user
password=${BWC_PW}
EOF
chmod 600 /root/.bwc-backup.cnf

# 6. Maintenance HTML
cat > /var/www/bwc-maint.html <<'EOF'
...
sudo -u bwc pm2 install pm2-logrotate || true
sudo -u bwc pm2 set pm2-logrotate:max_size 50M
sudo -u bwc pm2 set pm2-logrotate:retain 30
sudo -u bwc pm2 set pm2-logrotate:compress true
sudo -u bwc pm2 set pm2-logrotate:rotateInterval '0 0 * * *'
env PATH=$PATH:/usr/bin pm2 startup systemd -u bwc --hp /home/bwc
...
# 13. /etc/bwc/env.<env> templates (operator fills in)
...
NODE_ENV=production
PORT=3040
DATABASE_URL=mysql://bwc_user:${BWC_PW}@127.0.0.1:3306/bwc
DATABASE_MIGRATOR_URL=mysql://bwc_migrator:${MIG_PW}@127.0.0.1:3306/bwc
...
**Files:** Create `scripts/install-nginx.sh`, `scripts/nginx/bwc.conf.template`, `scripts/nginx/snippets/bwc-proxy.conf`, `scripts/nginx/snippets/bwc-tls.conf`, `scripts/nginx/snippets/bwc-compression.conf`

- [ ] **Step 1: proxy snippet**

```nginx
# scripts/nginx/snippets/bwc-proxy.conf
...
```nginx
# scripts/nginx/snippets/bwc-tls.conf
...
```nginx
# scripts/nginx/snippets/bwc-compression.conf
gzip on;
gzip_vary on;
gzip_comp_level 6;
...
```nginx
# scripts/nginx/bwc.conf.template — variables substituted by install-nginx.sh:
...
server {
  ...
  satisfy any;
  # allow lines populated by operator post-install (see /etc/nginx/conf.d/bwc-staging-allow.conf)
  include /etc/nginx/conf.d/bwc-staging-allow.conf;
  deny all;
  auth_basic "staging";
  ...
  location @maint_s { root /var/www; try_files /bwc-maint.html =503; }
...
if [ ! -f /etc/nginx/conf.d/bwc-staging-allow.conf ]; then
  echo "# Add 'allow <ip>;' lines here, one per office/dev IP." > /etc/nginx/conf.d/bwc-staging-allow.conf
fi
if [ ! -f /etc/nginx/.htpasswd-bwc-staging ]; then
  read -r -p "Staging basic auth user: " U
  htpasswd -c /etc/nginx/.htpasswd-bwc-staging "$U"
  chown root:www-data /etc/nginx/.htpasswd-bwc-staging
  chmod 640 /etc/nginx/.htpasswd-bwc-staging
fi
TMP="$(mktemp)"
awk -v apex="$APEX" -v staging="$STAGING" -v lp="$LOGIN_PATH" '{ gsub(/__APEX__/, apex); gsub(/__STAGING__/, staging); gsub(/__LOGIN_PATH__/, lp); print }' "$HERE/nginx/bwc.conf.template" > "$TMP"
...
**Files:** Create `scripts/install-systemd.sh`, `scripts/systemd/bwc-db-backup.service`, `scripts/systemd/bwc-db-backup.timer`, `scripts/systemd/bwc-uploads-backup.service`, `scripts/systemd/bwc-uploads-backup.timer`, `scripts/systemd/bwc-cron-purge.service`, `scripts/systemd/bwc-cron-purge.timer`, `scripts/systemd/bwc-disk-check.service`, `scripts/systemd/bwc-disk-check.timer`, `scripts/systemd/bwc-media-verify.service`, `scripts/systemd/bwc-media-verify.timer`, `scripts/systemd/bwc-alert@.service`, `scripts/systemd/bwc-static-prune.service`, `scripts/systemd/bwc-static-prune.timer`
...
[Unit]
Description=BWC nightly DB backup
...
ExecStart=/opt/bwc/current/scripts/db-backup.sh
```

```ini
# scripts/systemd/bwc-db-backup.timer
[Unit]
Description=BWC nightly DB backup timer

[Timer]
OnCalendar=*-*-* 03:00:00
RandomizedDelaySec=15min
Persistent=true
Unit=bwc-db-backup.service
...
[Unit]
Description=BWC nightly uploads rsync
OnFailure=bwc-alert@%n.service
...
```ini
# scripts/systemd/bwc-uploads-backup.timer
[Unit]
Description=BWC uploads backup timer

[Timer]
OnCalendar=*-*-* 04:30:00
RandomizedDelaySec=15min
Persistent=true
Unit=bwc-uploads-backup.service
...
[Unit]
Description=BWC nightly cron purge
After=bwc-uploads-backup.service
OnFailure=bwc-alert@%n.service
...
```ini
# scripts/systemd/bwc-cron-purge.timer
[Unit]
Description=BWC cron purge timer

[Timer]
OnCalendar=*-*-* 06:00:00
RandomizedDelaySec=15min
Persistent=true
Unit=bwc-cron-purge.service
...
```ini
# scripts/systemd/bwc-disk-check.service
[Unit]
Description=BWC hourly disk check

[Service]
Type=oneshot
User=root
ExecStart=/opt/bwc/current/scripts/disk-check.sh
```

```ini
# scripts/systemd/bwc-disk-check.timer
[Unit]
Description=BWC disk check timer

[Timer]
OnCalendar=hourly
Persistent=true
Unit=bwc-disk-check.service
...
```ini
# scripts/systemd/bwc-media-verify.service
[Unit]
Description=BWC weekly media-refs verifier
OnFailure=bwc-alert@%n.service
...
```ini
# scripts/systemd/bwc-media-verify.timer
[Unit]
Description=BWC media-verify timer

[Timer]
OnCalendar=Sun *-*-* 05:00:00
RandomizedDelaySec=15min
Persistent=true
Unit=bwc-media-verify.service
...
```ini
# scripts/systemd/bwc-alert@.service
[Unit]
Description=BWC OnFailure alert for %i

[Service]
Type=oneshot
ExecStart=/bin/bash -c 'journalctl -u %i --no-pager -n 50 | mailx -s "[bwc] %i failed" "$ADMIN_ALERT_EMAIL"'
EnvironmentFile=/etc/bwc/env.production
```
...
[Unit]
Description=BWC static-pool pruner

[Service]
Type=oneshot
User=bwc
ExecStart=/opt/bwc/current/scripts/prune-static-pool.sh
```

```ini
# scripts/systemd/bwc-static-prune.timer
[Unit]
Description=BWC static-pool pruner timer

[Timer]
OnCalendar=*-*-* 05:30:00
RandomizedDelaySec=10min
Persistent=true
Unit=bwc-static-prune.service
...
for t in bwc-db-backup bwc-uploads-backup bwc-cron-purge bwc-disk-check bwc-media-verify bwc-static-prune; do
...
LATEST_DUMP_KB=$(ls -1S /backup/bwc/db/*.sql.age 2>/dev/null | head -1 | xargs -I{} du -sk {} | awk '{print $1}' 2>/dev/null || echo 0)
...
[ ! -f /var/lib/bwc/deploy.blocked ] || { echo "preflight: /var/lib/bwc/deploy.blocked exists"; exit 1; }
...
# Mutex
exec 9<>/var/lock/bwc-deploy.lock
flock -n 9 || { echo "another deploy in progress"; exit 1; }

# Rotate: keep last 5 successful releases
ls -1dt /opt/bwc/releases/*/ 2>/dev/null | tail -n +6 | xargs -r -I{} rm -rf {}

# Unpack artifact pre-uploaded to /opt/bwc/incoming/<sha>.tar.zst
mkdir -p "$RELEASE_DIR"
tar --use-compress-program=zstd -xf "/opt/bwc/incoming/$SHA.tar.zst" -C "$RELEASE_DIR"
...
SNAP="/backup/bwc/pre-deploy/$SHA.sql.age"
mysqldump --defaults-extra-file=/root/.bwc-backup.cnf \
  --single-transaction --quick --skip-lock-tables --routines --triggers --events --set-gtid-purged=OFF bwc \
  | gzip | age -r "$(cat /etc/bwc/backup.pub)" > "${SNAP}.partial"
...
(
  cd "$RELEASE_DIR"
  DATABASE_URL="$(grep ^DATABASE_MIGRATOR_URL /etc/bwc/env."$ENV_NAME" | cut -d= -f2-)" \
    node migrator-bundle/run.js
)

# Merge static into pool (additive)
rsync -a "$RELEASE_DIR/.next/standalone/.next/static/" /opt/bwc/static-pool/.next/static/ || true
...
# PM2 reload
sudo -u bwc pm2 startOrReload "$RELEASE_DIR/ecosystem.config.cjs" --update-env
...
# Persist PM2
sudo -u bwc pm2 save || true

# Record success
echo "$SHA ok $(date -u +%FT%TZ)" >> /opt/bwc/releases-history.log
echo "deployed $SHA"
```
...
CURRENT="$(readlink /opt/bwc/current | awk -F/ '{print $NF}')"
LAST_OK="$(awk '$2=="ok" && $1!="'"$CURRENT"'" {sha=$1} END {print sha}' /opt/bwc/releases-history.log)"
if [ -z "$LAST_OK" ]; then echo "no prior successful release"; exit 1; fi

# Compare schema fingerprints
CUR_FP="$(cat /opt/bwc/releases/$CURRENT/db/schema-fingerprint.txt 2>/dev/null || echo missing)"
PREV_FP="$(cat /opt/bwc/releases/$LAST_OK/db/schema-fingerprint.txt 2>/dev/null || echo missing)"

ln -sfn "/opt/bwc/releases/$LAST_OK" /opt/bwc/current
sudo -u bwc pm2 startOrReload "/opt/bwc/releases/$LAST_OK/ecosystem.config.cjs" --update-env
...
if [ "$CUR_FP" != "$PREV_FP" ]; then
  ...
  if [ "$ans" = "y" ]; then
    ...
    IDENT="$(cat)"
    SNAP="/backup/bwc/pre-deploy/$CURRENT.sql.age"
    AGE_KEY=$(mktemp); echo "$IDENT" > "$AGE_KEY"
    age --decrypt -i "$AGE_KEY" "$SNAP" | gunzip | mysql --defaults-extra-file=/root/.bwc-migrate.cnf bwc
    shred -u "$AGE_KEY"
  fi
fi
...
cat > /root/.bwc-migrate.cnf <<EOF
[client]
host=127.0.0.1
user=bwc_migrator
password=${MIG_PW}
EOF
chmod 600 /root/.bwc-migrate.cnf
```
...
set -euo pipefail; IFS=$'\n\t'
DATE="$(date +%F)"
OUT="/backup/bwc/db/${DATE}.sql.age"
exec 9<>/var/lock/bwc-backup.lock
flock 9
ionice -c idle mysqldump --defaults-extra-file=/root/.bwc-backup.cnf \
  --single-transaction --quick --skip-lock-tables --routines --triggers --events --set-gtid-purged=OFF bwc \
  | gzip | age -r "$(cat /etc/bwc/backup.pub)" > "${OUT}.partial"
for s in "${PIPESTATUS[@]}"; do
  [ "$s" = 0 ] || { rm -f "${OUT}.partial"; echo "backup failed"; exit 1; }
done
# Size anomaly guard
NEW_SIZE=$(stat -c %s "${OUT}.partial")
YESTERDAY="/backup/bwc/db/$(date -d 'yesterday' +%F).sql.age"
if [ -f "$YESTERDAY" ]; then
...
mv "${OUT}.partial" "$OUT"
# Retention
find /backup/bwc/db -type f -name '*.sql.age' -mtime +30 -delete
# Off-site (optional)
if [ -n "${BACKUP_RCLONE_REMOTE:-}" ]; then
  rclone copy "$OUT" "$BACKUP_RCLONE_REMOTE/db/" --config /etc/bwc/rclone.conf
  rclone delete --min-age 90d "$BACKUP_RCLONE_REMOTE/db/" --config /etc/bwc/rclone.conf || true
fi
...
ionice -c idle rsync -a --delete-after \
  /opt/bwc/uploads/originals/         /backup/bwc/uploads/originals/
ionice -c idle rsync -a --delete-after \
  /opt/bwc/uploads/variants/          /backup/bwc/uploads/variants/
ionice -c idle rsync -a --delete-after \
  /opt/bwc/uploads/brochures-private/ /backup/bwc/uploads/brochures-private/
...
read -r -p "Scratch DB name [bwc_drill]: " DB; DB="${DB:-bwc_drill}"
...
done < <(ls -1dt /opt/bwc/releases/*/ 2>/dev/null | head -5)
...
if [ "$PCT" -ge 90 ]; then
  logger -t bwc-disk "CRITICAL disk ${PCT}%"
  install -d -o root -g root -m 750 /var/lib/bwc
  touch /var/lib/bwc/deploy.blocked
elif [ "$PCT" -ge 80 ]; then
  logger -t bwc-disk "WARN disk ${PCT}%"
fi
```
...
async function main() {
  ...
  for (const m of candidates) {
    ...
    const targets = m.mime_type === 'application/pdf'
      ? [`/opt/bwc/uploads/brochures-private/${m.filename_uuid}.pdf`]
      : [
          `/opt/bwc/uploads/originals/${m.filename_uuid}`,
          `/opt/bwc/uploads/variants/${m.filename_uuid}-thumb.webp`,
          `/opt/bwc/uploads/variants/${m.filename_uuid}-md.webp`,
          `/opt/bwc/uploads/variants/${m.filename_uuid}-lg.webp`,
          `/opt/bwc/uploads/variants/${m.filename_uuid}-og.jpg`,
        ]
...
jobs:
  test:
    ...
    services:
      mariadb:
        image: mariadb:10.11
        env: { MARIADB_ROOT_PASSWORD: root, MARIADB_DATABASE: bwc_test }
        ports: ['3306:3306']
    ...
    steps:
      ...
      - run: pnpm lint
      - env:
          DATABASE_URL: mysql://root:root@127.0.0.1:3306/bwc_test
    ...
    steps:
      ...
      - name: Run deploy.sh on droplet
        ...
        with:
          ...
          username: deployer
          key: ${{ secrets.DEPLOY_SSH_KEY }}
          script: sudo /opt/bwc/current/scripts/deploy.sh ${{ github.sha }} staging
```
    ...
    steps:
      ...
      - name: Bundle artifact
        run: |
          ...
          tar --use-compress-program=zstd -cf out/${SHA}.tar.zst -C out/${SHA} .
      - uses: appleboy/scp-action@v0.1.7
        with: { host: ${{ secrets.DROPLET_HOST }}, username: deployer, key: ${{ secrets.DEPLOY_SSH_KEY }}, source: "out/${{ github.sha }}.tar.zst", target: "/opt/bwc/incoming/" }
      - uses: appleboy/ssh-action@v1.0.3
        with:
          host: ${{ secrets.DROPLET_HOST }}
          username: deployer
          key: ${{ secrets.DEPLOY_SSH_KEY }}
          script: sudo /opt/bwc/current/scripts/deploy.sh ${{ github.sha }} production
```
...
```bash
ssh deployer@droplet sudo /opt/bwc/current/scripts/deploy.sh <sha> production
```

## Backups + restore

- DB: encrypted (age) nightly dump in `/backup/bwc/db/`, optional off-site via rclone.
- Uploads: rsync nightly to `/backup/bwc/uploads/`.
...
See `docs/specs/2026-05-10-bwc-rebuild-design.md` §15.
...
- 502 on the public site → check `pm2 logs bwc`. Maintenance page should be visible during deploy.
...
- [ ] systemd timers run on schedule; `OnFailure=bwc-alert@%n.service` emails admin.
