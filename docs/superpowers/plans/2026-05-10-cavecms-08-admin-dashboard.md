# CaveCMS Plan 08 — Admin Dashboard
...
export const GET = withError(async () => {
  ...
  return new Response(stream, {
    headers: {
      ...
      'content-disposition': `attachment; filename="cavecms-leads-${new Date().toISOString().slice(0, 10)}.csv"`,
...
export const POST = withError(async (req: Request) => {
  ...
  c.set('__Host-cavecms_reauth_at', String(Math.floor(Date.now() / 1000)), {
...
export async function requireFreshReauth(): Promise<void> {
  const c = await cookies()
  const ts = Number(c.get('__Host-cavecms_reauth_at')?.value ?? 0)
...
test('viewer cannot reach lead detail; sees masked list', async ({ page }) => {
  await loginAs(page, 'viewer@cavecms.test', 'ViewerPassword12!')
...
test('editor cannot toggle published; admin can', async ({ page }) => {
  await loginAs(page, 'editor@cavecms.test', 'EditorPassword12!')
...
test('last admin cannot be demoted', async ({ page, request }) => {
  await loginAs(page, 'admin@cavecms.test', 'CorrectHorseBattery0!')
