...
// POST /api/cms/pages/[id]/redo — step the page's draft overlay forward one
// revision (redoDraft). A draft redo is a state-changing mutation, so it
...
export const POST = withError<RouteCtx>(async (req, { params }) => {
  ...
  try {
    const { ok, draftVersion } = await redoDraft(id, ctx.userId)
